  ...
  return (
    <div className="container max-w-5xl py-4 md:py-8">
      <PageHeader
        ...
        actions={
          <Button asChild>
            <Link href={`/${store.slug}/admin/projects/new`}>New project</Link>
          </Button>
        }
      />
      {!projects || projects.length === 0 ? (
        <div className="mt-8 rounded-lg border border-dashed bg-muted/20 p-12 text-center text-sm text-muted-foreground">
          No projects yet. Use <span className="font-medium">New project</span> to
        ...
        <ul className="mt-6 grid gap-4 sm:grid-cols-2">
          {projects.map((project) => (
            <li key={project.id}>
              <Link href={`/${store.slug}/admin/projects/${project.id}`}>
